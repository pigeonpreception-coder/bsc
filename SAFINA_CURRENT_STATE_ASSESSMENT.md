...
| UX | 52 (52) | No change |
| DevSecOps | 58 (55) | gitleaks added on top of CodeQL/Dependabot — full git-history secrets scanning, not just the current tree. Not full marks — still no DAST (no staging URL to point it at), and none of the three tools has actually caught anything yet since they only just landed |
| Compliance | 10 (10) | No change |
| Globalization | 5 (5) | No change |
| AI Readiness | 60 (60) | No change |
| Enterprise Readiness | 37 (37) | No change |

**Overall maturity score: ~50/100, up from ~35/100 at the original baseline (~50 after the eleventh revision).** DevSecOps again — gitleaks rounds out the automated-scanning trio (SAST, SCA, secrets) started last revision. Everything else held flat, as expected for a tooling addition rather than a fix.
...
36. This update — twelfth revision, reflecting fix 35
...
**Operational note:** none of the three tools has produced a finding yet, since all three only just landed — there hasn't been a scan run against the current `main` at the time of writing. Check the repository's **Security → Code scanning alerts** tab (CodeQL) and **Actions** tab (gitleaks, which fails the workflow run rather than posting to Code scanning) after the next push to confirm they actually ran and produced results — not just that the workflow files are syntactically valid, which was verified locally, but that real runs completed successfully in GitHub's environment.
