...
**Baseline document, updated 2026-08-13 (thirteenth revision).** Originally derived from direct inspection of this repository as of 2026-08-12; revised thirteen times to reflect 38 commits made against its own findings (26 fixes plus these 13 revisions) (see §13 for the full list). Fixes 1–18 came from this document's own original gap analysis; fixes 19–20 from a second pass (§14); fix 21 from a third pass (§15, the confidentiality leak); fix 22 from a fourth pass (§16) — four passes, four distinct new categories of finding, the strongest evidence in this document that manual scanning alone doesn't converge on "done." Fixes 23–24 acted on that evidence: automated SAST (CodeQL), dependency scanning (Dependabot), and secrets scanning (gitleaks) are wired into CI (§17). Fix 25 is a **fifth manual pass anyway** (§18) — run after the tooling landed, specifically to test whether automated scanning had actually closed the gap. It found 5 more issues, none of a kind CodeQL/Dependabot/gitleaks would have caught (an unthrottled AI-cost-abuse vector, a cookie-security default, session non-revocation on password change) — the tooling and the manual-pass discipline are complementary, not substitutes for each other. Everything below reflects the codebase's actual current state, re-verified against the same evidence standard as the original: source, migrations, config, and test output — not assumptions. Where something couldn't be verified from available evidence — including whether a manual configuration step the user still needs to perform (e.g. creating a Sentry account) has actually been done — that's stated explicitly. Nothing here is a compliance or certification claim; none exist. [SECURITY_ARCHITECTURE_ASSESSMENT.md](SECURITY_ARCHITECTURE_ASSESSMENT.md) is a point-in-time snapshot from earlier the same session and is **not** kept in sync with this document going forward — this file is the current source of truth.
...
| No automated tests | **Fixed** — 107 tests across 17 files, covering the scoring/cascade engine, tenant isolation, cron auth, rate limiting, invite rollback, notifications, email, audit logging, plan-period math, storage-path/IP ownership guards, redirect-path validation, HTML-escaping, and the concurrency helper | Was P1 |
...
| Security | 73 (70) | Three concrete pass-5 fixes: AI-generation actions were the one class of Server Action with zero rate limiting (unlimited Anthropic-cost abuse potential) — now throttled; session cookies now explicitly set `secure`; a password change now revokes other active sessions. None as severe as the confidentiality leak or open redirect, but a real, felt narrowing of the attack surface. Still missing MFA, centralized authz, DAST |
...
| UX | 52 (52) | No change |
| DevSecOps | 58 (58) | No change — this pass was the pointed counter-experiment to last revision's tooling addition: run a manual pass anyway and see if CodeQL/Dependabot/gitleaks had already caught everything. They hadn't; none of pass 5's 5 findings were things that tooling would have surfaced. The tools stay valuable for what they do catch, but this is evidence the manual-pass discipline isn't obsolete, not evidence DevSecOps needs more tooling specifically |
| Compliance | 10 (10) | No change |
| Globalization | 5 (5) | No change |
| AI Readiness | 60 (60) | No change |
| Enterprise Readiness | 37 (37) | No change |

**Overall maturity score: ~51/100, up from ~35/100 at the original baseline (~50 after the twelfth revision).** Security carries this revision, as it has most gap-scan revisions — the fifth pass's findings were more about narrowing residual attack surface (rate limiting, cookie flags, session hygiene) than closing a single severe hole, which is the expected shape once the more serious classes (confidentiality leak, open redirect, SSRF) are already fixed.
...
Everything from the original list is done, error monitoring is code-complete, the notification system (in-app + email) is built, `user_permissions` is resolved (dropped), the gap-scan backlog through §18 is cleared, and SAST/SCA/secrets scanning now run automatically in CI (§17). What's left is entirely things this document can't do on its own — a verification step outside the repo, or a business decision:
...
38. This update — thirteenth revision, reflecting fix 37
