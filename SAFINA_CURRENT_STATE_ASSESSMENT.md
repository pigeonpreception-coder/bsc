...
**Baseline document, updated 2026-08-13 (ninth revision).** Originally derived from direct inspection of this repository as of 2026-08-12; revised nine times to reflect 29 commits made against its own findings (21 fixes plus these 9 revisions) (see §13 for the full list). Fixes 1–18 came from this document's own original gap analysis; fixes 19–20 came from a second, fresh gap-scan pass (§14); fix 21 came from a **third pass** (§15) — the most serious finding of the three passes so far, a genuine cross-tenant document-confidentiality leak, only surfaced on this third look. That's the pattern worth internalizing: each fresh pass has found something the previous ones missed, including passes run specifically to check for exactly that. There's no reason to assume a fourth pass wouldn't find a ninth issue — the absence of a known bug is not evidence of no bug. Everything below reflects the codebase's actual current state, re-verified against the same evidence standard as the original: source, migrations, config, and test output — not assumptions. Where something couldn't be verified from available evidence — including whether a manual configuration step the user still needs to perform (e.g. creating a Sentry account) has actually been done — that's stated explicitly. Nothing here is a compliance or certification claim; none exist. [SECURITY_ARCHITECTURE_ASSESSMENT.md](SECURITY_ARCHITECTURE_ASSESSMENT.md) is a point-in-time snapshot from earlier the same session and is **not** kept in sync with this document going forward — this file is the current source of truth.
...
| Control | Status |
|---|---|
| Tenant isolation (RLS) | **Implemented**, consistent across schema. Six gaps found across three scan passes, all fixed: `getCorporateBscView` trusting caller pre-scoping (pass 1); `addScorecardColumn`/`updateCellValue`/`assignPosition` trusting a caller-supplied foreign-key ID without verifying tenant ownership (pass 2, §14); `addScorecardRow` — same class, missed in pass 2 (pass 3, §15); and `buildUploadedDocumentContext` — a **read**, not a write: the RLS-bypassing admin client downloaded a caller-supplied storage path with no tenant-ownership check, the most severe finding across all three passes since it's a genuine cross-tenant confidentiality leak, not just an integrity gap (pass 3, §15). Each pass has found something the previous ones missed — treat "no known gaps" as current-evidence, not proof of absence |
...
| No automated tests | **Fixed** — 85 tests across 14 files, covering the scoring/cascade engine, tenant isolation, cron auth, rate limiting, invite rollback, notifications, email, audit logging, plan-period math, storage-path ownership, and the concurrency helper | Was P1 |
...
| Architecture | 58 (57) | `addScorecardRow` now checks tenant ownership consistently with its siblings; AI cascade generation now surfaces insert failures instead of silently reporting success on an empty scorecard |
| Security | 65 (61) | The largest jump of any single pass: closed a genuine cross-tenant **document-confidentiality leak** (`buildUploadedDocumentContext`, §15) — not just an integrity gap like the earlier write-path findings, an actual cross-tenant data exposure — plus the same-class `addScorecardRow` gap missed in pass 2. Still missing MFA, centralized authz, SAST/DAST |
| Privacy | 28 (25) | First movement in this domain all session: the document-confidentiality fix is a real privacy control, not just a security one — a tenant's uploaded documents can no longer be read into another tenant's AI output. Still no DSR workflow, no DPIA, no documented lawful basis |
| Scalability | 46 (46) | No change — the one scalability-shaped finding this pass (admin page N+1) was investigated and deliberately left as is (see §6/§15) |
| Performance | 40 (40) | No change — still no load-test evidence either way |
| Reliability | 50 (48) | The remaining unchecked cron-job writes (`daily_tasks`, `weekly_advisories`, `performance_history`, the `performance_scores` delete-before-insert) now check their errors — closes the last silent-failure gaps found across all three gap-scan passes |
| Data Architecture | 70 (70) | No change |
| UX | 52 (52) | No change |
| DevSecOps | 40 (40) | No change — three passes of manual gap-scanning found real issues, which is itself an argument for automated SAST/DAST, but manual passes aren't a substitute for the tooling this score is measuring |
| Compliance | 10 (10) | No change |
| Globalization | 5 (5) | No change |
| AI Readiness | 60 (60) | No change |
| Enterprise Readiness | 37 (37) | No change |

**Overall maturity score: ~48/100, up from ~35/100 at the original baseline (~47 after the eighth revision).** The jump this revision is concentrated almost entirely in Security and, for the first time, Privacy — the third gap-scan pass found the single most severe issue of the whole session (a cross-tenant document leak), which is exactly the kind of finding that moves those two domains specifically. Everything else held flat or moved only slightly, which is the expected shape of a scan-and-fix pass rather than a feature pass.
...
30. This update — ninth revision, reflecting fix 29
