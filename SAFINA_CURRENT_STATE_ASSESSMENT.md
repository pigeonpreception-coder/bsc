...
**Baseline document, updated 2026-08-13 (eleventh revision).** Originally derived from direct inspection of this repository as of 2026-08-12; revised eleven times to reflect 33 commits made against its own findings (23 fixes plus these 11 revisions) (see §13 for the full list). Fixes 1–18 came from this document's own original gap analysis; fixes 19–20 from a second pass (§14); fix 21 from a third pass (§15, the confidentiality leak); fix 22 from a fourth pass (§16) — four passes, four distinct new categories of finding, the strongest evidence in this document that manual scanning alone doesn't converge on "done." Fix 23 acts on that evidence directly: **automated SAST (CodeQL) and dependency scanning (Dependabot) are now wired into CI** instead of relying on another manual pass (§17) — DAST was considered and deliberately deferred, since there's no staging URL yet and most of the app sits behind auth, making a scan of limited value until then. Everything below reflects the codebase's actual current state, re-verified against the same evidence standard as the original: source, migrations, config, and test output — not assumptions. Where something couldn't be verified from available evidence — including whether a manual configuration step the user still needs to perform (e.g. creating a Sentry account) has actually been done — that's stated explicitly. Nothing here is a compliance or certification claim; none exist. [SECURITY_ARCHITECTURE_ASSESSMENT.md](SECURITY_ARCHITECTURE_ASSESSMENT.md) is a point-in-time snapshot from earlier the same session and is **not** kept in sync with this document going forward — this file is the current source of truth.
...
**DevOps:** **Fixed** — a CI pipeline now exists (`.github/workflows/ci.yml`: typecheck + lint + tests on every PR and push to `main`), and an automated test suite exists (Vitest, 104 tests across 17 files covering the scoring/cascade engine, tenant isolation, cron auth, rate limiting, the shared invite helper, audit logging, plan-period math, storage-path/IP ownership guards, redirect-path validation, HTML-escaping, and the notification/email helpers). True RLS/Postgres integration testing remains out of scope — no Supabase CLI/Docker access in this environment to run a local Postgres instance against the real policies; what exists tests the application-layer logic that supplements RLS. **Fixed** — automated SAST now runs in CI (`.github/workflows/codeql.yml`, GitHub CodeQL, `security-extended` query suite, on push/PR to `main` plus a weekly schedule — free for this public repo) and dependency/SCA scanning via Dependabot (`.github/dependabot.yml`, weekly, npm + GitHub Actions) — see §17. DAST deliberately not added yet: no staging URL exists to scan, and most of the app sits behind auth, so a scan against an unauthenticated instance would have limited value. Secrets via `process.env`, no secrets manager beyond Vercel's own env-var store.
...
| Automated security testing (SAST/dependency scanning) | **Fixed — SAST and SCA now in CI.** GitHub CodeQL (`security-extended`, catches SSRF/open-redirect/injection classes among others) and Dependabot (npm + GitHub Actions, weekly) — see §17. Neither has produced a real finding yet as of this writing (both only just landed); that's not the same claim as "nothing to find" |
...
| Architecture | 59 (59) | No change |
| Security | 70 (70) | No change in this revision specifically — the fixes are done; this revision is about detection capability going forward, not a new fix. Still missing MFA, centralized authz, DAST |
| Privacy | 28 (28) | No change |
...
| UX | 52 (52) | No change |
| DevSecOps | 55 (40) | The response to four passes each finding a category manual scanning was slow to catch: GitHub CodeQL (`security-extended`, SAST) and Dependabot (SCA) now run automatically in CI on every push/PR plus weekly, instead of relying on another manual pass. Not full marks — no DAST yet (no staging URL to point it at), no secrets-scanning tool, and neither new tool has actually caught anything yet since they only just landed |
| Compliance | 10 (10) | No change |
| Globalization | 5 (5) | No change |
| AI Readiness | 60 (60) | No change |
| Enterprise Readiness | 37 (37) | No change |

**Overall maturity score: ~50/100, up from ~35/100 at the original baseline (~49 after the tenth revision).** This revision's movement is entirely DevSecOps — the shift from "manual pass finds it, then we fix it" to "tooling runs continuously" is the more durable investment of the two, even though it hasn't (yet) found anything the manual passes didn't already catch first.
...
Everything from the original list is done, error monitoring is code-complete, the notification system (in-app + email) is built, `user_permissions` is resolved (dropped), the gap-scan backlog through §16 is cleared, and SAST/SCA now run automatically in CI (§17). What's left is entirely things this document can't do on its own — a verification step outside the repo, or a business decision:
...
Notification trigger points now cover four events (position assignment, weekly-advisory generation, new-account welcome, plan approval). Further ones (e.g. task overdue, KPI status change surfaced through the same channel as `performance_alerts`) remain additive, whenever a concrete need shows up. There is no gap-scan backlog waiting behind this one — CodeQL and Dependabot now watch continuously instead of needing another manual pass to catch the next category.
...
34. This update — eleventh revision, reflecting fix 33
...
- **Secrets scanning** (e.g. gitleaks, or GitHub's own secret-scanning feature) — not evaluated in this pass; a reasonable next addition, but not requested and not built speculatively.
- **Correctness bugs, business logic gaps, N+1 patterns, and defense-in-depth hardening** — SAST tools are good at recognizable vulnerability *shapes* (taint flows into a sink: SQL, redirect targets, SSRF, XSS). Several of the manual-pass findings across §14–§16 (the tenant-isolation write gaps, the unchecked cron-job errors, the N+1 loops) are business-logic-specific and wouldn't necessarily match a generic CodeQL/Semgrep rule. **This means the manual-pass discipline that found them isn't fully replaced by this tooling** — CodeQL and Dependabot are a real addition, not a like-for-like substitute for periodic manual review of this codebase's own specific invariants (does this write check tenant ownership? does this insert check its error?).

**Operational note:** neither tool has produced a finding yet, since both only just landed in this same commit — there hasn't been a scan run against the current `main` yet at the time of writing. Check the repository's **Security → Code scanning alerts** tab after the next push to confirm CodeQL actually ran and produced results (not just that the workflow file is syntactically valid, which was verified locally, but that a real run completed successfully in GitHub's environment).
